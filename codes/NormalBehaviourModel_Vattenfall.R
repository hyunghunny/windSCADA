...
dataloading <- function (turbine='WTG01',yr='2014',start=1,end) {
  ...
  windScada <- read.csv("./data/WH1111.csv")
  # Remove the data when power is below zero
  s.length<-length(windScada$WH1111.GearBearTempAvg)
  windScada <- subset(windScada,windScada$WH1111.ActivePower>0)[start:end,] 
#  windScada <- subset(windScada,windScada$WH1111.ActivePower<1500)
#  windScada <- subset(windScada,windScada$WH1111.ActivePower>0)[start:s.length,] 

  time <- strptime(windScada$UTC.Timestamp,"%Y-%m-%d %H:%M:%S" )
  oil.temp <- windScada$WH1111.GearOilTempAvg
  power <- windScada$WH1111.ActivePower
  ambient.temp <-windScada$WH1111.NacelleTempAvg
  bearing.temp <- windScada$WH1111.GearBearTempAvg
...
#  print(ggplot(output,aes(Time,Residual)) + geom_point(colour="green") + geom_line(aes(Time,amb.av),color="black")+labs(title=paste(turbine,'Bearing Temperature Residual from',output$Time[1],'to',output$Time[length(output$Time)])) + ylim(-10,10))
  ggplot(output,aes(Time,Residual)) + geom_point(colour="green") + geom_line(aes(Time,amb.av),color="black")+labs(title=paste(turbine,'Bearing Temperature Residual from',output$Time[1],'to',output$Time[length(output$Time)])) + ylim(-10,10)
  
  return(list(turbine=turbine,year=yr,output=output))
  
}

ann.model<-bearing.training('Vattenfall','Year',1000,3000)
Vattenfall.Output2010<-predict.bearing(turbine='WH1111', yr='2010',start=5000,10000)
